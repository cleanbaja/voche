...
export class DeepgramSTT implements Transcriber {
  ...
  constructor(bus: EventEmitter, platform: Platform) {
    ...
    this.cognitive.on(LiveTranscriptionEvents.Open, () => {
      ...
      this.cognitive.on(LiveTranscriptionEvents.Close, (ev: CloseEvent) => {
      ...
      this.cognitive.on(LiveTranscriptionEvents.Metadata, (data: MessageEvent) => {
        logger.info("metadata: " + data);
      });

      this.cognitive.on(LiveTranscriptionEvents.Error, (err: ErrorEvent) => {
        logger.error(err);
      });
    });
  }
...
export class DeepgramTTS implements Synthesizer {
  ...
  constructor(bus: EventEmitter, platform: Platform) {
    ...
    bus.on("neural:data", async (data: string) => {
